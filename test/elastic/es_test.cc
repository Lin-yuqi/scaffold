...
const std::string index_type="_doc";
const std::string url="http://elastic:123456@192.168.86.129:9200/";

void create(const linelastic::ESClient::ptr &client) {
    linelastic::Indexer indexer(index_name);
...
//2. 新增数据
void insert(const linelastic::ESClient::ptr &client) {
    linelastic::Inserter inserter(index_name, "1");
...
// 3. 更新数据
void update1(const linelastic::ESClient::ptr &client) {
    linelastic::Updater updater(index_name, "1");
    updater.add("name", "李四");
...
void update2(const linelastic::ESClient::ptr &client) {
    linelastic::Inserter inserter(index_name, "1");
...
// 4. 查询数据
void select1(const linelastic::ESClient::ptr &client) {
    linelastic::Searcher searcher(index_name);
    auto query = searcher.query();
    query->match("name")->setValue("王五");
    searcher.from(0);
    searcher.size(1);
    ...
    std::cout << *linutil::JSON::serialize(*res) << std::endl;
}
void select2(const linelastic::ESClient::ptr &client) {
    linelastic::Searcher searcher(index_name);
    auto query = searcher.query();
...
// 5. 删除数据
void remove1(const linelastic::ESClient::ptr &client) {
    linelastic::Deleter deleter(index_name, "1");
...
// 6. 删除索引 
void remove2(const linelastic::ESClient::ptr &client) {
...
int main()
{
    ...
    auto client = std::make_shared<linelastic::ESClient>(urls);
    //create(client);
