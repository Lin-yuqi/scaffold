...
int main(){
    ...
    const std::string delayed_exchange="delayed_exchange";
    const std::string delayed_queue="delayed_queue";
    const std::string delayed_routing_key="delayed_routing_key";
...
//  实例化channel对象
    ...
    channel.declareExchange(delayed_exchange,AMQP::ExchangeType::direct,0)
    .onSuccess([&](){
        std::cout<<"交换机声明成功"<<std::endl;
        //  声明队列
        AMQP::Table args;
        ...
        args["x-message-ttl"]=5000; // 消息过期时间5秒
        channel.declareQueue(delayed_queue,args)
            .onSuccess([&](const std::string &name, uint32_t messagecount, uint32_t consumercount){
                std::cout<<"队列声明成功，队列名:"<<name<<std::endl;
                std::cout<<"队列中消息数:"<<messagecount<<",消费者数:"<<consumercount<<std::endl;
                //  绑定队列到交换机
                channel.bindQueue(delayed_exchange,delayed_queue,delayed_routing_key)
                    .onSuccess([&](){
                        std::cout<<"队列绑定到交换机成功"<<std::endl;
                        //  发布消息到交换机
                        std::string message="Hello, DLX with AMQP-CPP!";
                        bool ret=channel.publish(delayed_exchange,delayed_routing_key,message);
                        if(!ret){
