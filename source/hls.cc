...
bool M3U8Info::parse() {
    std::string body;
    bool ret=linutil::FUTIL::read(_filename, body);
    if(!ret){
        ERR("Failed to read m3u8 file: {}", _filename.c_str());
        return false;
    }

    std::vector<std::string> str_list;
    int count=linutil::STR::split(body, "\n", str_list);
...
bool M3U8Info::write() {
    ...
    ss << HLS_ENDLIST;
    bool ret = linutil::FUTIL::write(_filename, ss.str());
    if (ret == false) {
...
std::vector<M3U8Info::StrPair>& M3U8Info::pieces() {
    return _pieces;
}
...
bool HLSTranscoder::transcode(const std::string &input, const std::string &output) {
    ...
    //5. 设置HLS转码的各项细节参数：播放类型-点播vod，分片时间， 路径前缀 http://192.168.65.130:9000/video/
